...
画像を一括生成し、ギャラリーとして管理、さらにローカル/クラウド LLM でセリフを付け、
...
- **セリフ生成** — 画像ごとに、その状況でキャラが言いそうなセリフをローカル LLM（Ollama）または
  Grok（xAI）で生成・編集。
...
- **Ollama**（ローカル LLM・無検閲セリフ生成）
  アダルト表現を含むセリフはクラウド LLM では拒否されることがあるため、ローカルモデルを推奨。

  ```sh
  # 例: 日本語ロールプレイ向けモデルのセットアップ（初回のみ）
  ...
  # 設定 → 文章生成(LLM) → モデル名: ninja-nsfw-rp
  ```

  （素の GGUF はチャットテンプレートを持たないため、`models/Ninja-v1-NSFW-RP.Modelfile` で
  `[INST] … [/INST]` 形式を登録しています。詳細は同ファイル冒頭のコメント参照。）

- **xAI Grok API キー** — クラウドでセリフ/記事文を生成したい場合（露骨な表現は拒否され得る）。
...
Ollama（ローカル LLM）の URL・モデル名・セリフ用プロンプトテンプレートも設定画面で調整します。
...
- **コミットされる**: ソース、アプリアイコン（`build/`・`resources/icon.png`）、
  局部検出モデル（`resources/models/*.onnx`）、Ollama Modelfile。
